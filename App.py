...
if st.session_state.bilans:
    ...
    with col_exp1:
        ...
        st.download_button("📊 Télécharger pour Excel (CSV)", data=csv, file_name="bilan_projet.csv", mime="text/csv")

    # 2. Export PDF (Sécurisé)
    with col_exp2:
        def generate_pdf(data_df):
            ...
            for _, row in data_df.iterrows():
                for val in row:
                    # Remplacement du symbole € par EUR pour éviter l'erreur latin-1
                    txt = str(val).replace('\u20ac', 'EUR').replace('€', 'EUR')
                    pdf.cell(38, 10, txt.encode('latin-1', 'replace').decode('latin-1'), border=1, align='C')
                pdf.ln()
